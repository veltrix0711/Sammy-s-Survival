...
public class ItemRecipeDesignerWindow : EditorWindow
{
	...
	private ItemCategory itemCategory = ItemCategory.Generic;
	private BaseMaterial itemMaterial = BaseMaterial.Unknown;
	private ItemCapability itemCapabilities = ItemCapability.None;
	...
	private void DrawItemsTab()
	{
		...
		itemCategory = (ItemCategory)EditorGUILayout.EnumPopup("Category", itemCategory);
		itemMaterial = (BaseMaterial)EditorGUILayout.EnumPopup("Material", itemMaterial);
		itemCapabilities = (ItemCapability)EditorGUILayout.EnumFlagsField("Capabilities", itemCapabilities);
		EditorGUILayout.LabelField("Default Condition", EditorStyles.boldLabel);
		itemDefaultCondition.dirt = EditorGUILayout.Slider("Dirt", itemDefaultCondition.dirt, 0, 1);
		itemDefaultCondition.wetness = EditorGUILayout.Slider("Wetness", itemDefaultCondition.wetness, 0, 1);
		itemDefaultCondition.rust = EditorGUILayout.Slider("Rust", itemDefaultCondition.rust, 0, 1);
		itemDefaultCondition.paintWear = EditorGUILayout.Slider("Paint Wear", itemDefaultCondition.paintWear, 0, 1);
		itemDefaultCondition.sharpness = EditorGUILayout.Slider("Sharpness", itemDefaultCondition.sharpness, 0, 1);
		itemDefaultCondition.integrity = EditorGUILayout.Slider("Integrity", itemDefaultCondition.integrity, 0, 1);
	...
	private ItemDefinition CreateItem()
	{
		...
		so.FindProperty("category").enumValueIndex = (int)itemCategory;
		so.FindProperty("material").enumValueIndex = (int)itemMaterial;
		so.FindProperty("capabilities").intValue = (int)itemCapabilities;
		so.FindProperty("defaultCondition").FindPropertyRelative("dirt").floatValue = itemDefaultCondition.dirt;
		so.FindProperty("defaultCondition").FindPropertyRelative("wetness").floatValue = itemDefaultCondition.wetness;
		so.FindProperty("defaultCondition").FindPropertyRelative("rust").floatValue = itemDefaultCondition.rust;
		so.FindProperty("defaultCondition").FindPropertyRelative("paintWear").floatValue = itemDefaultCondition.paintWear;
		so.FindProperty("defaultCondition").FindPropertyRelative("sharpness").floatValue = itemDefaultCondition.sharpness;
		so.FindProperty("defaultCondition").FindPropertyRelative("integrity").floatValue = itemDefaultCondition.integrity;
